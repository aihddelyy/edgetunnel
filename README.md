...
| 🛡️ **反代优先级修复** | 修复优选 IP 作为反代 IP 与个性化 PROXYIP 冲突的问题。当优选 IP 携带 `proxyip=true` 时，优先使用优选 IP 作为反代 IP；仅当优选 IP 未配置为反代时，才回退到个性化 PROXYIP。链式代理（`$socks5://...` 等）同样受保护。优先级：**链式代理 > 优选 IP 作为反代 IP > 个性化 PROXYIP**。 |
| 🔗 **反代来源隔离** | 修复同地址节点被错误反代的问题。`proxyip=true` 的反代范围按来源 API 备注隔离——只有同一 `sub=...#备注 proxyip=true` 来源的同地址节点才走反代，不会污染其他同名节点。 |
